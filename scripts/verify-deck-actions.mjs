...
const missing=key('application',{},'Unknown App');
assert.ok(a.actionIssue(missing)); assert.equal(a.deviceKeycode(missing,0),null);
const automatic=key('application',{},'Open Photoshop');
assert.equal(a.actionIssue(automatic),null); assert.deepEqual(a.applicationExecutables(automatic),['Photoshop.exe']);
assert.equal(a.applicationSearchMode(key('application',{appPath:'C:\\Adobe\\Photoshop.exe'},'Open Photoshop')),'manual');
assert.deepEqual(a.applicationExecutables(key('application',{appExecutable:'Portable.exe'},'Open Photoshop')),['Portable.exe']);
assert.equal(a.normalizedDriveLetters('c, D c'),'C,D'); assert.equal(a.normalizedDriveLetters('C,D,E,F,G,H,I'),null);
assert.ok(a.actionIssue(key('application',{appSearch:'auto',appExecutable:'bad/name.exe'},'Unknown App')));
assert.ok(a.actionIssue(key('application',{appSearch:'auto',appExecutable:'bad/name.exe'},'Open Photoshop')));
assert.ok(a.actionIssue(key('application',{appSearch:'auto',appExecutable:'portable.exe',appDeepSearch:true,appDrives:'C,?'},'Unknown App')));
...
const app=key('application',{appSearch:'manual',appPath:'C:\\Program Files\\Photoshop.exe'});
const autoApp=key('application',{appSearch:'auto',appExecutable:'Portable App.exe',appDeepSearch:true,appDrives:'C,D'},'Unknown App');
const generated=a.windowsScript([assign(web,0),assign(app,1),assign(macro,12),assign(autoApp,2),assign(missing,3),assign(key('keyboard',{shortcut:'Ctrl+C'}),4)]);
...
assert.ok(generated.source.includes('Run(Chr(34) appPath Chr(34))'));
assert.match(generated.source,/AI_PAD_FindApp\(\["Portable App\.exe"\], "C,D"\)/);
assert.match(generated.source,/RegRead\(root name\)/); assert.ok(generated.source.includes('Loop Files, drive ":\\" name, "R"'));

...
import { LED_ALERTS, LED_SCENARIOS, resolveKineLeds, type LedScenario, type LedSettings } from './kine-led';
...
export default function KineLedControls({ settings, connected, onChange, onCloseup }: {
  ...
  return <div className="kine-led-panel">
    <div className="kine-led-heading"><div><span>LED · LIGHT STUDY</span><strong>{state.simulated ? '화면 시뮬레이션' : 'USB 응답 기준 · 영상 재현'}</strong></div><button onClick={onCloseup}>LED 확대 ↗</button></div>
    ...
    <fieldset className="kine-led-alerts"><legend>작업 알림 미리보기 · 화면 전용</legend><div>{LED_ALERTS.map(alert => <button key={alert.scenario} aria-pressed={settings.scenario === alert.scenario} onClick={() => onChange({ scenario: alert.scenario })}><i style={{ background: alert.color }}/>{alert.label}</button>)}<button onClick={() => onChange({ scenario: 'auto' })}>알림 해제</button></div><p>자동 알림·기기 LED 제어는 아직 연결되지 않았습니다. <a href="/guide#led-notifications">가능한 연동 방법 보기 ↗</a></p></fieldset>

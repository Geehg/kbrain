...
const contents = [
  ['start','빠른 시작'], ['connect','연결 방법'], ['keys','조합키'], ['via','VIA 설정'],
  ['layout','플레이트 배열'], ['firmware','펌웨어'], ['package','구성품'],
];
  ...
  return (
    <main className="guide-shell">
      ...
      <section className="guide-hero">
        <div className="hero-copy"><p className="guide-eyebrow">NOVA KINE / USER GUIDE 01</p><h1>처음 연결부터<br/>키맵 설정까지.</h1><p>공식 설명서의 핵심 절차를 한국어로 다시 구성했습니다. 먼저 USB-C로 설정을 완료한 뒤 Bluetooth 또는 2.4GHz로 전환하는 흐름을 권장합니다.</p><div className="hero-actions"><a href="#start">5분 빠른 시작</a><a href={manualUrl} target="_blank" rel="noreferrer">공식 PDF ↗</a></div></div>
        <div className="hero-device" aria-label="NOVA KINE 키 배열 개념도">
          <div className="mini-board"><div className="mini-keys">{Array.from({length:24},(_,index)=><span key={index} className={index===3?'fn-key':''}>{index===3?'FN':''}</span>)}</div><div className="mini-dock"><i/><i/><i/><b/></div></div>
      ...
      <div className="guide-layout">
        ...
        <article className="guide-content">
          <section id="start" className="guide-section">
            <div className="section-number">01</div><div className="section-copy"><p className="guide-eyebrow">QUICK START</p><h2>5분 빠른 시작</h2><div className="step-list">
          ...
          <section id="connect" className="guide-section">
            <div className="section-number">02</div><div className="section-copy"><p className="guide-eyebrow">CONNECTION</p><h2>연결 방법</h2><div className="connection-cards">
          ...
          <section id="keys" className="guide-section">
            <div className="section-number">03</div><div className="section-copy"><p className="guide-eyebrow">KEY COMBINATIONS</p><h2>꼭 기억할 조합키</h2><div className="combo-grid">
          ...
          <section id="via" className="guide-section">
            <div className="section-number">04</div><div className="section-copy"><p className="guide-eyebrow">VIA CONFIGURATION</p><h2>VIA 키맵 설정</h2><div className="via-flow"><span>JSON 다운로드</span><i>→</i><span>Design에서 로드</span><i>→</i><span>Test Matrix</span><i>→</i><span>Configure</span></div>
          ...
          <section id="firmware" className="guide-section">
            <div className="section-number">06</div><div className="section-copy"><p className="guide-eyebrow">FIRMWARE UPDATE</p><h2>BIN 펌웨어 업데이트</h2><ol className="firmware-list"><li>LUMINKEY 자료실에서 NOVA KINE용 BIN 파일을 내려받습니다.</li><li>왼쪽 위 <kbd>Print</kbd> 키를 누른 상태로 USB-C 케이블을 연결합니다.</li><li>컴퓨터에 나타난 이동식 USB 드라이브에 BIN 파일을 복사합니다.</li><li>진행이 완료되고 드라이브가 자동으로 사라질 때까지 기다립니다.</li><li>표시등이 다시 켜지면 케이블을 분리했다가 다시 연결합니다.</li></ol><div className="danger"><b>업데이트 중 케이블을 분리하지 마세요.</b><p>드라이브가 자동으로 사라지기 전에 케이블을 빼거나 창을 강제로 닫으면 업데이트에 실패할 수 있습니다. 제품에 맞는 제조사 BIN 파일만 사용하세요.</p></div></div>
          </section>

          <section id="package" className="guide-section">
            <div className="section-number">07</div><div className="section-copy"><p className="guide-eyebrow">IN THE BOX</p><h2>기본 구성품</h2><div className="package-grid">{[['NOVA KINE','1'],['데이터 케이블','1'],['교체 플레이트','3'],['2.4G 수신기','1'],['1U 여분 키캡','6'],['여분 스위치','3'],['T6 드라이버','1'],['여분 나사 세트','1'],['키캡·스위치 풀러','1'],['빠른 시작 안내서','1']].map(([name,count])=><span key={name}><b>{name}</b><em>× {count}</em></span>)}</div></div>
          </section>
